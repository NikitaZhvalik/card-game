...
it('renderPage is called with duplicatedCards', () => {
  const renderPage = jest.fn();
  const cards = shuffleCards(5, ['2', '3', '4', '5', '6'], ['hearts', 'diamonds', 'clubs', 'spades'], renderPage);
  expect(renderPage).toHaveBeenCalledWith(cards);
});
